...
class UsageMetricsProcessor {
  ...
  static UsageDerivedMetrics derive(DailyUsageStats daily) {
    ...
    final generalUsage = (totalAppUsage - productiveUsage - entertainmentUsage)
        .clamp(0, totalAppUsage);

    final weightedProductiveUsage =
        productiveUsage + (generalUsage * _generalProductiveWeight);
    final productiveRatio = totalAppUsage > 0
        ? (weightedProductiveUsage / totalAppUsage).clamp(0.0, 1.0)
        : 0.0;

    final screenHours =
        daily.screenTimeMs / const Duration(hours: 1).inMilliseconds;
    final lateNightHours =
        daily.lateNightUsageMs / const Duration(hours: 1).inMilliseconds;

    // Allow more intentional usage before mild penalties kick in.
    final screenThresholdHours = 4.0 + (1.5 * productiveRatio);
    final screenOveruse = (screenHours - screenThresholdHours).clamp(
      0.0,
      double.infinity,
    );
    ...
    final expectedUnlocks =
        (12.0 + (screenHours * (12.0 - (2.0 * productiveRatio)))).clamp(
          20.0,
          90.0,
        );
    final unlockOveruse = (daily.unlockCount - expectedUnlocks).clamp(
      0.0,
      double.infinity,
    );
    ...
    final lateNightOveruse = (lateNightHours - lateNightThresholdHours).clamp(
      0.0,
      double.infinity,
    );

    final screenPenalty = _softPenalty(
      screenOveruse,
      pivot: 2.0,
      maxPenalty: _screenPenaltyWeight,
    );
    final unlockPenalty = _softPenalty(
      unlockOveruse,
      pivot: 35.0,
      maxPenalty: _unlockPenaltyWeight,
    );
    final latePenalty = _softPenalty(
      lateNightOveruse,
      pivot: 1.2,
      maxPenalty: _lateNightPenaltyWeight,
    );
    ...
    final dominantRatio = totalAppUsage > 0
        ? dominant.value / totalAppUsage
        : 0.0;
    final addiction =
        (100.0 *
